...
// 2026-09 の第1週: [月外, 09-01 (火) 〜 09-06 (日)]
...
describe("weekBars", () => {
  it("週にかかるイベントを列範囲の帯にする", () => {
    expect(weekBars(week, [event("2026-09-05", "2026-09-06")])).toEqual([
      { event: event("2026-09-05", "2026-09-06"), startCol: 5, span: 2, lane: 0 },
    ]);
  });
  ...
  it("週の途中から始まる月またぎイベントは月内の日だけに切り詰める", () => {
    ...
    expect(bars).toEqual([{ event: event("2026-08-31", "2026-09-01"), startCol: 1, span: 1, lane: 0 }]);
  });
  ...
  it("重なるイベントは別レーンに積み、空いたレーンは再利用する", () => {
    ...
    expect(bars).toEqual([
      { event: long, startCol: 1, span: 3, lane: 0 },
      { event: short, startCol: 1, span: 1, lane: 1 },
      { event: later, startCol: 5, span: 1, lane: 0 },
    ]);
  });
});
